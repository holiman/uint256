...
func leadingZeros(x *Int) (z int) {
	var t int
	z = bits.LeadingZeros64(x.value[3])
	t = bits.LeadingZeros64(x.value[2])
	if z == 64 {
		z = t + 64
	}
	t = bits.LeadingZeros64(x.value[1])
	if z == 128 {
		z = t + 128
	}
	t = bits.LeadingZeros64(x.value[0])
	if z == 192 {
		z = t + 192
	}
	return z
...
//
// Notes:
// - specialized for m.value[3] != 0, hence limited to 2^192 <= m < 2^256
// - returns zero if m.value[3] == 0
...
func Reciprocal(m *Int) (mu [5]uint64) {
	...
	s := bits.LeadingZeros64(m.value[3]) // Replace with leadingZeros(m) for general case
	p := 255 - s                         // floor(log_2(m)), m>0

	// 0 or a power of 2?

	// Check if at least one bit is set in m.value[2], m.value[1] or m.value[0],
	// or at least two bits in m.value[3]

	if m.value[0]|m.value[1]|m.value[2]|(m.value[3]&(m.value[3]-1)) == 0 {
	...
	t1 := uint64(r0)                   // 2^31/y
	t1 *= t1                           // 2^62/y^2
	t1, _ = bits.Mul64(t1, y.value[3]) // 2^62/y^2 * 2^64/y / 2^64 = 2^62/y
	...
	b2h, _ := bits.Mul64(a2l, y.value[2])
	c2h, c2l := bits.Mul64(a2l, y.value[3])
	d2h, d2l := bits.Mul64(a2h, y.value[2])
	e2h, e2l := bits.Mul64(a2h, y.value[3])
	...
	q0, _ = bits.Mul64(x2, y.value[0])
	q1, t0 = bits.Mul64(x3, y.value[0])
	q0, c = bits.Add64(q0, t0, 0)
	q1, _ = bits.Add64(q1, 0, c)

	t1, _ = bits.Mul64(x1, y.value[1])
	q0, c = bits.Add64(q0, t1, 0)
	q2, t0 = bits.Mul64(x3, y.value[1])
	q1, c = bits.Add64(q1, t0, c)
	q2, _ = bits.Add64(q2, 0, c)

	t1, t0 = bits.Mul64(x2, y.value[1])
	q0, c = bits.Add64(q0, t0, 0)
	q1, c = bits.Add64(q1, t1, c)
	q2, _ = bits.Add64(q2, 0, c)

	t1, t0 = bits.Mul64(x1, y.value[2])
	q0, c = bits.Add64(q0, t0, 0)
	q1, c = bits.Add64(q1, t1, c)
	q3, t0 = bits.Mul64(x3, y.value[2])
	q2, c = bits.Add64(q2, t0, c)
	q3, _ = bits.Add64(q3, 0, c)

	t1, _ = bits.Mul64(x0, y.value[2])
	q0, c = bits.Add64(q0, t1, 0)
	t1, t0 = bits.Mul64(x2, y.value[2])
	q1, c = bits.Add64(q1, t0, c)
	q2, c = bits.Add64(q2, t1, c)
	q3, _ = bits.Add64(q3, 0, c)

	t1, t0 = bits.Mul64(x1, y.value[3])
	q1, c = bits.Add64(q1, t0, 0)
	q2, c = bits.Add64(q2, t1, c)
	q4, t0 = bits.Mul64(x3, y.value[3])
	q3, c = bits.Add64(q3, t0, c)
	q4, _ = bits.Add64(q4, 0, c)

	t1, t0 = bits.Mul64(x0, y.value[3])
	q0, c = bits.Add64(q0, t0, 0)
	q1, c = bits.Add64(q1, t1, c)
	t1, t0 = bits.Mul64(x2, y.value[3])
	q2, c = bits.Add64(q2, t0, c)
	...
	x1, x0 = bits.Mul64(d4h, y.value[0])
	x3, x2 = bits.Mul64(f4h, y.value[0])
	t1, t0 = bits.Mul64(f4l, y.value[0])
	x1, c = bits.Add64(x1, t0, 0)
	x2, c = bits.Add64(x2, t1, c)
	x3, _ = bits.Add64(x3, 0, c)

	t1, t0 = bits.Mul64(d4h, y.value[1])
	x1, c = bits.Add64(x1, t0, 0)
	x2, c = bits.Add64(x2, t1, c)
	x4, t0 := bits.Mul64(f4h, y.value[1])
	x3, c = bits.Add64(x3, t0, c)
	x4, _ = bits.Add64(x4, 0, c)
	t1, t0 = bits.Mul64(d4l, y.value[1])
	x0, c = bits.Add64(x0, t0, 0)
	x1, c = bits.Add64(x1, t1, c)
	t1, t0 = bits.Mul64(f4l, y.value[1])
	x2, c = bits.Add64(x2, t0, c)
	x3, c = bits.Add64(x3, t1, c)
	x4, _ = bits.Add64(x4, 0, c)

	t1, t0 = bits.Mul64(a4h, y.value[2])
	x0, c = bits.Add64(x0, t0, 0)
	x1, c = bits.Add64(x1, t1, c)
	t1, t0 = bits.Mul64(d4h, y.value[2])
	x2, c = bits.Add64(x2, t0, c)
	x3, c = bits.Add64(x3, t1, c)
	x5, t0 := bits.Mul64(f4h, y.value[2])
	x4, c = bits.Add64(x4, t0, c)
	x5, _ = bits.Add64(x5, 0, c)
	t1, t0 = bits.Mul64(d4l, y.value[2])
	x1, c = bits.Add64(x1, t0, 0)
	x2, c = bits.Add64(x2, t1, c)
	t1, t0 = bits.Mul64(f4l, y.value[2])
	x3, c = bits.Add64(x3, t0, c)
	x4, c = bits.Add64(x4, t1, c)
	x5, _ = bits.Add64(x5, 0, c)

	t1, t0 = bits.Mul64(a4h, y.value[3])
	x1, c = bits.Add64(x1, t0, 0)
	x2, c = bits.Add64(x2, t1, c)
	t1, t0 = bits.Mul64(d4h, y.value[3])
	x3, c = bits.Add64(x3, t0, c)
	x4, c = bits.Add64(x4, t1, c)
	x6, t0 := bits.Mul64(f4h, y.value[3])
	x5, c = bits.Add64(x5, t0, c)
	x6, _ = bits.Add64(x6, 0, c)
	t1, t0 = bits.Mul64(a4l, y.value[3])
	x0, c = bits.Add64(x0, t0, 0)
	x1, c = bits.Add64(x1, t1, c)
	t1, t0 = bits.Mul64(d4l, y.value[3])
	x2, c = bits.Add64(x2, t0, c)
	x3, c = bits.Add64(x3, t1, c)
	t1, t0 = bits.Mul64(f4l, y.value[3])
	x4, c = bits.Add64(x4, t0, c)
	...
	q1, q0 = bits.Mul64(x0, y.value[0])
	q3, q2 = bits.Mul64(x2, y.value[0])
	q5, q4 := bits.Mul64(x4, y.value[0])

	t1, t0 = bits.Mul64(x1, y.value[0])
	q1, c = bits.Add64(q1, t0, 0)
	q2, c = bits.Add64(q2, t1, c)
	t1, t0 = bits.Mul64(x3, y.value[0])
	q3, c = bits.Add64(q3, t0, c)
	q4, c = bits.Add64(q4, t1, c)
	q5, _ = bits.Add64(q5, 0, c)

	t1, t0 = bits.Mul64(x0, y.value[1])
	q1, c = bits.Add64(q1, t0, 0)
	q2, c = bits.Add64(q2, t1, c)
	t1, t0 = bits.Mul64(x2, y.value[1])
	q3, c = bits.Add64(q3, t0, c)
	q4, c = bits.Add64(q4, t1, c)
	q6, t0 := bits.Mul64(x4, y.value[1])
	q5, c = bits.Add64(q5, t0, c)
	q6, _ = bits.Add64(q6, 0, c)

	t1, t0 = bits.Mul64(x1, y.value[1])
	q2, c = bits.Add64(q2, t0, 0)
	q3, c = bits.Add64(q3, t1, c)
	t1, t0 = bits.Mul64(x3, y.value[1])
	q4, c = bits.Add64(q4, t0, c)
	q5, c = bits.Add64(q5, t1, c)
	q6, _ = bits.Add64(q6, 0, c)

	t1, t0 = bits.Mul64(x0, y.value[2])
	q2, c = bits.Add64(q2, t0, 0)
	q3, c = bits.Add64(q3, t1, c)
	t1, t0 = bits.Mul64(x2, y.value[2])
	q4, c = bits.Add64(q4, t0, c)
	q5, c = bits.Add64(q5, t1, c)
	q7, t0 := bits.Mul64(x4, y.value[2])
...
//
// requires a four-word modulus (m.value[3] > 1) and its inverse (mu)
func reduce4(x [8]uint64, m *Int, mu [5]uint64) (z Int) {
	...
	r4, r3 = bits.Mul64(q0, m.value[3])
	_, t0 = bits.Mul64(q1, m.value[3])
	r4, _ = bits.Add64(r4, t0, 0)

	t1, r2 = bits.Mul64(q0, m.value[2])
	r3, c = bits.Add64(r3, t1, 0)
	_, t0 = bits.Mul64(q2, m.value[2])
	r4, _ = bits.Add64(r4, t0, c)

	t1, t0 = bits.Mul64(q1, m.value[2])
	r3, c = bits.Add64(r3, t0, 0)
	r4, _ = bits.Add64(r4, t1, c)

	t1, r1 = bits.Mul64(q0, m.value[1])
	r2, c = bits.Add64(r2, t1, 0)
	t1, t0 = bits.Mul64(q2, m.value[1])
	r3, c = bits.Add64(r3, t0, c)
	r4, _ = bits.Add64(r4, t1, c)

	t1, t0 = bits.Mul64(q1, m.value[1])
	r2, c = bits.Add64(r2, t0, 0)
	r3, c = bits.Add64(r3, t1, c)
	_, t0 = bits.Mul64(q3, m.value[1])
	r4, _ = bits.Add64(r4, t0, c)

	t1, r0 = bits.Mul64(q0, m.value[0])
	r1, c = bits.Add64(r1, t1, 0)
	t1, t0 = bits.Mul64(q2, m.value[0])
	r2, c = bits.Add64(r2, t0, c)
	r3, c = bits.Add64(r3, t1, c)
	_, t0 = bits.Mul64(q4, m.value[0])
	r4, _ = bits.Add64(r4, t0, c)

	t1, t0 = bits.Mul64(q1, m.value[0])
	r1, c = bits.Add64(r1, t0, 0)
	r2, c = bits.Add64(r2, t1, c)
	t1, t0 = bits.Mul64(q3, m.value[0])
	r3, c = bits.Add64(r3, t0, c)
	r4, _ = bits.Add64(r4, t1, c)
	...
	if b != 0 {
		r0, c = bits.Add64(r0, m.value[0], 0)
		r1, c = bits.Add64(r1, m.value[1], c)
		r2, c = bits.Add64(r2, m.value[2], c)
		r3, c = bits.Add64(r3, m.value[3], c)
	...
	for {
		// q = r - m
		q0, b = bits.Sub64(r0, m.value[0], 0)
		q1, b = bits.Sub64(r1, m.value[1], b)
		q2, b = bits.Sub64(r2, m.value[2], b)
		q3, b = bits.Sub64(r3, m.value[3], b)
	...
	z.value[3], z.value[2], z.value[1], z.value[0] = r3, r2, r1, r0
