...
func leadingZeros(x Int) (z int) {
	...
	z += bits.LeadingZeros64(x[2])
	if z < 128 {
		return
	}
	z += bits.LeadingZeros64(x[1])
	if z < 192 {
		return
	}
	z += bits.LeadingZeros64(x[0])
	return z
}
...
func reciprocal(m Int) (mu [5]uint64) {
	...
	for w := 0; w < cacheWays; w++ {
		if cache[cacheIndex].mod[w][3] != m[3] {
			continue
		}
		if cache[cacheIndex].mod[w][2] != m[2] {
			continue
		}
		if cache[cacheIndex].mod[w][1] != m[1] {
			continue
		}
		if cache[cacheIndex].mod[w][0] != m[0] {
			continue
		}
